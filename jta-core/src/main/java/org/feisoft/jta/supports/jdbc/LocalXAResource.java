...
public class LocalXAResource implements XAResource {
	...
	public void recoverable(Xid xid) throws XAException {
		...
		try {
			...
			sql.append("select xid, gxid, bxid from jta where xid = ? gxid = ? and bxid = ? ");
	...
	public synchronized void end(Xid xid, int flags) throws XAException {
		if (xid == null) {
			...
			if (this.loggingRequired) {
				...
				PreparedStatement stmt = null;
				try {
					stmt = connection.prepareStatement("insert into jta(xid, gxid, bxid, ctime) values(?, ?, ?, ?)");
					stmt.setString(1, identifier);
					stmt.setString(2, gxid);
					stmt.setString(3, bxid);
	...
	protected boolean isTableExists(Connection conn) throws SQLException {
		...
		try {
			...
			rs = metadata.getTables(catalog, schema, "jta", null);
			return rs.next();
		} finally {
			this.closeQuietly(rs);
		}
