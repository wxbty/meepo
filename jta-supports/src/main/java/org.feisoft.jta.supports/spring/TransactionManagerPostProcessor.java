...
public class TransactionManagerPostProcessor implements BeanFactoryPostProcessor {
	...
	public void postProcessBeanFactory(ConfigurableListableBeanFactory beanFactory) throws BeansException {
		...
		for (int i = 0; i < beanNameArray.length; i++) {
			...
			if (org.feisoft.jta.supports.jdbc.LocalXADataSource.class.equals(beanClass)) {
				beanNameList.add(beanName);
			} else if (org.feisoft.jta.UserTransactionImpl.class.equals(beanClass)) {
				beanNameList.add(beanName);
				userTransactionBeanName = beanName;
			} else if (org.feisoft.jta.TransactionManagerImpl.class.equals(beanClass)) {
		...
		if (transactionManagerBeanName == null) {
			throw new FatalBeanException("No configuration of class org.feisoft.bytetcc.TransactionManagerImpl was found.");
		}
