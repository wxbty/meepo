...
public class SqlpraserUtils {
    ...
    public static Image handleRollBack(BackInfo backInfo, List<String> list, Connection connection, Statement stmt) {
...
//                backSql.addAll(handleInsert(rollsql));
                ...
                Image imgae = handleUpdate(backInfo,rollsql, stmt, connection);
                return imgae;
...
//                backSql.addAll(handleDelete(rollsql, connection));
    ...
    public static Image handleUpdate(BackInfo backInfo, String updateSql, Statement stmt, Connection conn) {
        ...
        try {
            ...
            String[] strs = getPrimaryKey(conn, tables.get(0));
            String pkey = strs[0];
            ...
            backInfo.setSelectBody("select " + columns + " from " + table);
            backInfo.setSelectWhere(" where " + whereSql);
            backInfo.setChangeType("update");
            backInfo.setChangeSql(updateSql);
            ...
            List<LineFileds> line = new ArrayList<>();

            for (Map<String, Object> peMap : key_value_list) {
                LineFileds lf = new LineFileds();
                List<org.bytesoft.bytejta.image.Field> fileds = new ArrayList<>();
                        StringBuffer backSql = new StringBuffer();
                for (String col : peMap.keySet()) {
                    org.bytesoft.bytejta.image.Field field = new org.bytesoft.bytejta.image.Field(col, peMap.get(col).getClass().getName(), peMap.get(col));
                    fileds.add(field);
                }
                lf.setFields(fileds);
                line.add(lf);
            }
            beforeImage.setIsbefore(Boolean.TRUE);
            beforeImage.setLine(line);
            beforeImage.setTableName(table);
            beforeImage.setSchemaName(strs[1]);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public static String[] getPrimaryKey(Connection con, String tableName) throws Exception {
        ...
        return new String[]{primaryKeyColumnName,schemasName};
    }
